...
#import pipeline                         #dont have to redefine functions
                                        #the idiot who wrote this is an idiot
...
def print_help2():
    print('  ╔══════════════════════════════════════════════════════════════════════╗')
    print('  ║  Please make sure [maskStart] [maskEnd] are within boundaries of     ║')
    print('  ║        your sequence length. Please refer to above parameters.       ║')
    print('  ╚══════════════════════════════════════════════════════════════════════╝\n')
...
def sequence_mutin(seqaa, start, end):
    rate = 0.20         # Mutation rate
    aa = "ACDEFGHIKLMNPQRSTVWY"
    newSeq = ""
    for i in range(len(seq)):
        if i >= start and i <=end and random.uniform(0, 1) < rate:
...
def sequence_mutout(seqaa, start, end):
    rate = 0.20         # Mutation rate
    aa = "ACDEFGHIKLMNPQRSTVWY"
    newSeq = ""
    for i in range(len(seq)):
...
def initialization(path, basename):
    ...
    return length, objective, seqpb, seqaa
...
def init_generation(path, basename, threshold, length, seqpb, seqaa, start, end, maskio):
    ...
    while zScore < threshold:      # Initial sequence validation threshold is set to 3
        # Sequence generation
        if maskio == "Y" :
            seqaa2 = sequence_mutin(seqaa, start,end)
        if maskio == "N" :
            seqaa2 = sequence_mutout(seqaa, start,end)
    ...
    return zScore, seqaa2


# First process of sequences generation
def init_process(path, basename, initRun, pool, threshold, length, bestScore, seqpb, seqaa, start, end, maskio):
    ...
    with tqdm_joblib(desc="", total=pool) as progress_bar:
        data += Parallel(n_jobs=-1)(delayed(init_generation)(path, basename, threshold, length, seqpb, seqaa, start, end, maskio) for i in range(pool))
...
def following_generation(path, basename, seqaa, seqN, nbCopies, seqpb):
    ...
    return oriSeq, zScore, seqaa


# All but first mutation/scoring cycles (Cycles 1 and over).
def following_processes(path, basename, newRun, runIter, sequences, bestScore, seqpb, start, end, maskio):
    batch = []
    nbCopies = 100
    ...
    for i in tqdm(range(len(sequences))):
        ...
        batch.append(Parallel(n_jobs=-1)(delayed(sequence_mutin)(sequences[i], start, end) for j in range(nbCopies)))
    ...
    with tqdm_joblib(desc="", total=len(batch)) as progress_bar:
        data.append(Parallel(n_jobs=-1)(delayed(following_generation)(path, basename, batch[i], i, nbCopies, seqpb) for i in range(len(batch))))
