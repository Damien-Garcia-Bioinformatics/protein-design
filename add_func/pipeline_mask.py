...
def init_generation(path, basename, threshold, length, seqpb):
    seqaa = ""
    zScore = 0.0
    id = uuid.uuid4()
...
def init_process(path, basename, initRun, pool, threshold, length, bestScore, seqpb):
    ...
    with tqdm_joblib(desc="", total=pool) as progress_bar:
        data += Parallel(n_jobs=-1)(delayed(init_generation)(path, basename, threshold, length, seqpb) for i in range(pool))
...
def following_processes(path, basename, newRun, runIter, sequences, bestScore,seqpb):
    batch = []
    nbCopies = 100
...
if __name__ == "__main__":
    ...
    seqpb = ""
    pool = 100
    length, objective, seqpb = initialization(path, basename)
    ...
    while bestScore < objective and runIter < 5:
        if runIter == 0:
            initRun = init_process(path, basename, initRun, pool, threshold, length, bestScore, seqpb)
            bestScore = max(initRun['zScore'].to_list())
        elif runIter == 1:
            sequences = initRun['sequence'].to_list()
            newRun = following_processes(path, basename, newRun, runIter, sequences, bestScore, seqpb)
            bestScore = max(initRun['zScore'].to_list())
            print(newRun)
        else:
